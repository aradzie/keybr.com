import { compose } from "./ast.ts";
import _keywords from "./syntax/_keywords.ts";
import lang_cpp from "./syntax/lang_cpp.ts";
import lang_html_css from "./syntax/lang_html_css.ts";
import lang_javascript from "./syntax/lang_javascript.ts";
import lang_python from "./syntax/lang_python.ts";
import lang_regex from "./syntax/lang_regex.ts";
import lang_rust from "./syntax/lang_rust.ts";
import lang_shell from "./syntax/lang_shell.ts";

export const grammar_html_css = compose(lang_html_css, _keywords);
export const grammar_cpp = compose(lang_cpp, _keywords);
export const grammar_javascript = compose(lang_javascript, _keywords);
export const grammar_python = compose(lang_python, _keywords);
export const grammar_rust = compose(lang_rust, _keywords);
export const grammar_shell = compose(lang_shell);
export const grammar_regex = compose(lang_regex);
